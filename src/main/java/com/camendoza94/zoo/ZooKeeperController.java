        ...
        try {
            ...
            if (children.size() > 1 || !obtainURL(children.get(0)).equals(NO_SERVICE)) {
                for (String child : children) {
                    if (zooKeeperClientManager.getZNodeData(child, false) == 0) {//1) {
                        ...
                        try {
                            ...
                            for (Object key : body.keySet()) {
                                ...
                                Object keyvalue = body.get(keyStr);
                                String keyvalueFinal = keyvalue.toString().replaceAll("\"","");
                                //Print key and value
                                System.out.println("key: "+ keyStr + " value: " + keyvalueFinal);

                                URL = URL.replace(keyStr, keyvalueFinal);
                            }

                            return template.getForEntity ("https://" + URL, String.class, "");
                        } catch (Exception e) {
                    ...
                    for (String path : services) {
                        if (!path.isEmpty()) {
                            ...
                            for (String part : parts) {
                                if (!part.endsWith("/") && zooKeeperClientManager.getZNodeStats("/" + BASE_PATH + "/" + deviceId + "/" + part) == null)
